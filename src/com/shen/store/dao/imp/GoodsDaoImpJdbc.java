...
import com.shen.db.core.JdbcTemplate;
import com.shen.db.core.PreparedStatementCreator;
import com.shen.db.core.RowCallbackHandler;
...
public class GoodsDaoImpJdbc implements GoodsDao {
    ...
    public Goods findByPk(long pk) {
        ...
        String sql = "select id,name,price,description,brand,cpu_brand,cpu_type,memory_capacity,hd_capacity,card_model,displaysiza,image from Goods where id=?";
    ...
    public List<Goods> findAll() {
        ...
        String sql = "select id,name,price,description,brand,cpu_brand,cpu_type,memory_capacity,hd_capacity,card_model,displaysiza,image from Goods";
        jdbcTemplate.query(conn -> {
            PreparedStatement ps = conn.prepareStatement(sql);
            return ps;
        }, rs -> {
    ...
    public List<Goods> findStartEnd(int start, int end) {
        ...
        StringBuffer sql = new StringBuffer("select id,name,price,description,brand,cpu_brand,cpu_type,memory_capacity,hd_capacity,card_model,displaysiza,image from Goods");
    ...
    private void populate(ResultSet rs, List<Goods> list) throws SQLException {
        ...
        goods.setDisplaysize(rs.getString("displaysiza"));
    ...
    public void create(Goods goods) {
        ...
        String sql = "insert into Goods (id,name,price,description,brand,cpu_brand,cpu_type,memory_capacity,hd_capacity,card_model,displaysiza,image) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public void modify(Goods goods) {
        ...
        String sql = "update Goods set name=?,price=?,description=?,brand=?,cpu_brand=?,cpu_type=?,memory_capacity=?,hd_capacity=?,card_model=?,displaysiza=?,image=? where id=?";
        jdbcTemplate.update(conn -> {
